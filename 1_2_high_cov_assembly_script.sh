...
if [ ! -e $freads ]; then
filtlong --min_length 200 --keep_percent 99.99 $reads > $freads || exit 1
fi
...
for ass in flye miniasm raven necat; do # flye miniasm raven necat
	if [[ $ass == "flye" ]]; then #Assemble w/ Flye
		...
		for i in {1..3}; do
			if [ ! -e $out_dir/assemblies/$sample_name.$ass."$i".fasta ]; then
				...
				echo "Running $ass assembly $i"
				seqtk sample -s "$i" $freads "$read_count" | paste - - - - | shuf | tr '\t' '\n' > $sreads
				/home/wordenp/Minipolish/miniasm_and_minipolish.sh $sreads "$threads" 1> $workdir/assembly.gfa 2> $logs/$ass.$i.log.txt
				RESULT=$?
				if [ $RESULT -eq 0 ]; then
					echo "$ass assembly $i successful"
